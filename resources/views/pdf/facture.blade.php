...
.header-doc-number { font-size:19px; font-weight:700; color:#4ade80; font-family:monospace; text-align:right; margin-top:4px; }
.header-doc-date { font-size:9.5px; color:rgba(255,255,255,.6); text-align:right; margin-top:3px; }

/* ── Accent stripe + banner ── */
.accent-stripe { height:3px; background:#059669; }
.paid-banner { background:#059669; color:#fff; text-align:center; padding:9px; font-size:11px; font-weight:700; letter-spacing:3px; text-transform:uppercase; }
...
.sum-total-amount { color:#4ade80; }
.vat-note { font-size:8px; color:#94a3b8; text-align:right; font-style:italic; margin-bottom:14px; }

/* ── Versements ── */
.versements-box { background:#f0fdf4; border:1px solid #bbf7d0; border-radius:5px; padding:12px 14px; margin-bottom:18px; }
.versement-row-table { width:100%; border-collapse:collapse; }
.versement-row td { padding:6px 0; font-size:11px; border-bottom:1px solid #dcfce7; }
.versement-row:last-child td { border-bottom:none; }
.versement-amount { color:#16a34a; font-weight:700; text-align:right; }

/* ── Total réglé ── */
.paid-box {
    background:#f0fdf4; border:2px solid #059669; border-radius:6px;
    padding:14px; text-align:center; margin-bottom:18px;
}
.paid-label { font-size:9px; font-weight:700; text-transform:uppercase; letter-spacing:1.2px; color:#059669; margin-bottom:5px; }
.paid-amount { font-size:22px; font-weight:700; color:#065f46; }

/* ── Conditions ── */
.terms-box { background:#f8fafc; border-radius:4px; padding:10px 14px; margin-bottom:16px; font-size:10px; color:#475569; line-height:1.65; }
</style>
</head>
<body>
...
<div class="pdf-footer">
    ...
    <div class="footer-legal">
        ...
        <span class="footer-date">Facture émise le {{ now()->format('d/m/Y à H:i') }} · Ce document tient lieu de reçu et atteste du règlement complet de la commande {{ $docNumber ?? $order->order_number }}</span>
    </div>
</div>
...
<table class="header-table">
    <tr>
        ...
        <td style="width:45%;text-align:right;">
            <div class="header-doc-label">Facture</div>
            <div class="header-doc-number">{{ $docNumber ?? $order->order_number }}</div>
            <div class="header-doc-date">Émise le {{ now()->format('d/m/Y') }}</div>
...
<div class="paid-banner">✓ Payée — Livrée</div>
...
@endphp
    ...
    <table class="parties-table">
        <tr>
            <td style="width:48%;padding-right:8px;vertical-align:top;">
                <div class="party-box">
                    <div class="party-tag">Fournisseur</div>
                ...
                <div class="summary-box">
                    <table class="summary-row-table">
                        ...
                        <tr class="sum-row-total">
                            <td>{{ $vatRegime === 'standard' ? 'TOTAL TTC' : 'TOTAL' }}</td>
    ...
    <div class="section-label">Détail des paiements reçus</div>
    ...
    {{-- Total réglé --}}
    <div class="paid-box">
        <div class="paid-label">Montant total réglé</div>
        <div class="paid-amount">{{ number_format($order->total, 2, ',', ' ') }} {{ $currency }}</div>
    </div>

    {{-- Conditions de vente --}}
    @if(!empty($settings['doc_terms_text']))
    <div class="terms-box">
        <strong style="color:#1e293b;">Conditions de vente : </strong>{!! nl2br(e($settings['doc_terms_text'])) !!}
    </div>
    @endif
